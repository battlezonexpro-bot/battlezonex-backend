require("dotenv").config();
    ...
    const BACKEND_URL = process.env.BACKEND_URL || "https://battlezonex-backend.
  onrender.com";
    ...
    app.all("/send-notification", async (req, res) => {
      try {
        ...
        const big_picture = req.body.big_picture || req.query.big_picture || req.body.
  image;
        const url = req.body.url || req.query.url;
        let inputUids = req.body.uids || req.query.uids;

        if (!title || !message) return res.status(400).json({ status: false, message:
  "Missing title/message" });

        let parsedUids = null;
        if (inputUids) {
          if (typeof inputUids === 'string') {
            try { parsedUids = JSON.parse(inputUids); } catch (e) { parsedUids =
  [inputUids]; }
    ...
    app.post("/create-order", async (req, res) => {
      try {
        ...
        if (!uid || !amount || !customer_mobile) return res.status(400).json({
  status: false, message: "Missing fields" });

        const order_id = `BZX_uid.slice(0,8){Date.now()}`;
        const payload = {
          customer_mobile,
  ...
  U                i
    ...
    {order_id}`,
        ...
        const response = await axios.post("https://pay0.shop/api/create-order", qs.
  stringify(payload), {
          headers: { "Content-Type": "application/x-www-form-urlencoded" }
        });

        const payUrl = response.data.payment_url || (response.data.result && response.
  data.result.payment_url);
        if (response.data && (response.data.status === true || response.data.status
  === "SUCCESS") && payUrl) {
          await db.collection("PendingOrders").doc(order_id).set({
            order_id, uid, amount: Number(amount), status: "PENDING", createdAt:
  ...
  "Gateway Error" });
    ...
    app.all("/webhook", async (req, res) => {
      ...
      try {
        ...
        const checkRes = await axios.post("https://pay0.shop/api/check-order-status",
  qs.stringify({ user_token: PAY0_TOKEN, order_id }), {
          headers: { "Content-Type": "application/x-www-form-urlencoded" }
        });

        const apiData = checkRes.data || {};
        let isSuccess = (apiData.status === true || String(apiData.status).
  toUpperCase() === "SUCCESS");
        ...
        if (isSuccess) {
          ...
          await db.runTransaction(async (t) => {
            ...
            t.update(orderRef, { status: "CREDITED", creditedAt: admin.firestore.
  FieldValue.serverTimestamp() });
            const depositRef = db.collection("Deposits").doc(order_id);
            t.set(depositRef, { depositId: order_id, orderId: order_id, userId: uid,
  amount, status: "Confirmed", gateway: "Pay0", timestamp: Date.now() });
          });
          await sendNotification("Deposit Successful 💰", `₹${amount} added
  successfully to wallet`, [uid]);
        }
        res.send("OK");
    ...
    app.post("/join-match", async (req, res) => {
      ...
      if (!matchId || !uid || !ign) return res.status(400).json({ status: false,
  message: "Missing fields" });

      try {
        const matchQuery = await db.collection("Matches").where("matchId", "==",
  matchId).get();
        if (matchQuery.empty) return res.status(400).json({ status: false, message:
  "Match not found" });
        ...
        await db.runTransaction(async (t) => {
          ...
          if (!userDoc.exists) throw new Error("User profile not found! Please
  restart app.");
          const mData = matchDoc.data();
          matchDataForNotif = mData;
          if (mData.status !== "Upcoming") throw new Error("This match is no longer
  upcoming!");
          ...
          const players = Array.isArray(mData.joinedPlayers) ? [...mData.
  joinedPlayers] : [];
          const igns = Array.isArray(mData.joinedIGNs) ? [...mData.joinedIGNs] : [];

          while(igns.length < players.length) igns.push("Player");

          if (players.includes(uid)) throw new Error("You have already joined this
  match!");
          ...
          if (bon >= remFee) { bon -= remFee; remFee = 0; }
            ...
            if (dep >= remFee) { dep -= remFee; remFee = 0; }
              ...
              else throw new Error("Insufficient Balance! Please add money to your
  wallet.");
            }
          }

          t.update(userRef, { bonusBalance: bon, depositBalance: dep, winningBalance:
  win });
          players.push(uid);
          igns.push(ign);
          t.update(matchRef, { joinedSpots: joined + 1, joinedPlayers: players,
  joinedIGNs: igns });
        });

        res.json({ status: true, message: "Joined successfully!" });

        if (matchDataForNotif && matchDataForNotif.isPlayerHosted &&
  matchDataForNotif.hostUid && matchDataForNotif.hostUid !== uid) {
           await sendNotification("Player Joined!", `${ign} joined your match:
  ${matchDataForNotif.title}`, [matchDataForNotif.hostUid]);
    ...
    app.post("/claim-spin", async (req, res) => {
      ...
      if (!uid || bonusAmount == null) return res.status(400).json({ status: false,
  message: "Missing fields" });
    ...
    app.post("/request-withdrawal", async (req, res) => {
      ...
      if (!uid || !amount || !upiId) return res.status(400).json({ status: false,
  message: "Missing fields" });
