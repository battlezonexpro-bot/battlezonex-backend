...
app.post("/join-match", async (req, res) => {
  ...
  try {
    ...
    await db.runTransaction(async (t) => {
      ...
      // READS 1: Fetch Referrer if B (current user) is playing first paid match
      if (uData.referredBy && !uData.referralRewarded && initialFee > 0) {
          const rQuery = await t.get(db.collection("Users").where("myReferralCode", "==", uData.referredBy).limit(1));
          if (!rQuery.empty) {
              referrerDoc = rQuery.docs[0];
          }
      }

      // READS 2: Fetch all waiting referees if A (current user) is playing their first paid match
      if (!uData.hasPlayedMatch && initialFee > 0 && uData.myReferralCode) {
          const wQuery = await t.get(db.collection("Users").where("referredBy", "==", uData.myReferralCode));
          waitingReferees = wQuery.docs.filter(d => {
              const dData = d.data();
              return dData.referralRewarded === true && !dData.refereeRewarded;
          });
      }
      ...
      let updateUserData = { bonusBalance: bon, depositBalance: dep, winningBalance: win };
      if (initialFee > 0) {
      ...
      if (referrerDoc) {
          ...
          updateUserData.referralRewarded = true;
      ...
      // WRITES 2: Retroactively reward all B's if this is A's (current user) FIRST paid match
      if (waitingReferees.length > 0) {
          for (let wDoc of waitingReferees) {
              const wData = wDoc.data();
              const newBon = (Number(wData.bonusBalance) || 0) + refBonusAmount;
              t.update(wDoc.ref, { 
                  bonusBalance: newBon,
