...
app.all("/send-notification", async (req, res) => {
  try {
    ...
    const uids = req.query.uids || req.body.uids;
    const exclude_uids = req.query.exclude_uids || req.body.exclude_uids;
    ...
    let parsedUids = uids;
    let parsedExcludeUids = exclude_uids;

    if (typeof uids === 'string') parsedUids = [uids];
    if (typeof exclude_uids === 'string') parsedExcludeUids = [exclude_uids];

    await sendNotification(title, message, parsedUids, parsedExcludeUids);
